...
public class selectedCheckboxes {
    ...
    List<WebElement>Sectedboxes=driver.findElements(By.xpath("//input[@type = 'checkbox' and contains(@id, 'day')] "));
        for (WebElement days:Sectedboxes) {
            if(days.getAttribute("id").equalsIgnoreCase("tuesday")||(days.getAttribute("id").equalsIgnoreCase("thursday"))||(days.getAttribute("id").equalsIgnoreCase("saturday"))){
                days.click();
            }
